...
import { achievementData, catalogInfo, divisionFor, franchiseFor, infinityStones, orderEntries, presetMatches, searchCreditsFor, upcomingProjects, yearFor, type Profile, type UpcomingProject, type WatchOrder } from "./catalog";
...
const METADATA_VERSION = catalogInfo.version;
...
function withOverride(entry: WatchEntry, details: MediaDetails): MediaDetails {
  ...
  return { ...details, episodeTitle: override.title || details.episodeTitle, releaseDate: override.releaseDate || details.releaseDate || entry.releaseDate, genres: normalizeGenres(override.genres || details.genres || entry.genres), cast: override.cast || details.cast || entry.cast, description: override.description || details.description || entry.description };
}
...
export default function Home() {
  ...
  return <main>
    ...
    {view === "archive" && <>
      ...
      {!!upcomingProjects.length && <section className="upcoming-shell"><div className="section-title"><div><p className="eyebrow">On the horizon</p><h2>Upcoming releases</h2></div><span>Automatically monitored each morning</span></div><div className="upcoming-grid">{upcomingProjects.map((project) => { const days = today ? Math.ceil((new Date(`${project.date}T12:00:00`).valueOf() - today.valueOf()) / 86400000) : 0; return <article key={project.archiveId}><button className="upcoming-main" onClick={() => setSelectedUpcoming(project)}><PosterArt title={project.title} /><span><small>{project.type} · {displayDate(project.date)}</small><strong>{project.title}</strong><em>{days > 0 ? `${days} days` : "Release detected — steward check pending"}</em><b>View details →</b></span></button><a href={project.trailer} target="_blank" rel="noreferrer" aria-label={`Watch the ${project.title} trailer`}><Icon name="play" />Trailer</a></article>; })}</div></section>}
    ...
    {view === "settings" && <section className="inner-page settings-page">
      ...
      <div className="settings-grid">
        ...
        <article><div className="panel-heading"><h2>Catalog update center</h2><span>{catalogInfo.lastCheckedAt ? "Monitoring" : "Setup required"}</span></div><dl><div><dt>App version</dt><dd>{APP_VERSION}</dd></div><div><dt>Catalog version</dt><dd>{METADATA_VERSION}</dd></div><div><dt>Items indexed</dt><dd>{entries.length}</dd></div><div><dt>Upcoming monitored</dt><dd>{upcomingProjects.length}</dd></div><div><dt>Last steward check</dt><dd>{catalogInfo.lastCheckedAt ? new Date(catalogInfo.lastCheckedAt).toLocaleString() : "Not run yet"}</dd></div></dl><p className="update-note"><strong>Latest catalog activity</strong> {catalogInfo.changelog[0]?.detail || "No catalog changes recorded."}</p><p className="tmdb-credit"><a href="https://www.themoviedb.org" target="_blank" rel="noreferrer">TMDB</a> supplies automated release and metadata checks. This product uses the TMDB API but is not endorsed or certified by TMDB.</p></article>
        <article className="whats-new"><div className="panel-heading"><h2>What’s new in v14.4</h2></div><p>Infinity Archive Catalog Steward: scheduled release checks, safe automatic promotion, date and metadata refreshes, permanent managed IDs, validation, catalog history, and visible health status.</p><p>High-confidence future movies can be discovered through the exact Marvel Studios + MCU classification intersection. Television discovery waits for an episode-level importer.</p></article>
      </div>
    </section>}
    ...
    <footer><strong>THE INFINITY ARCHIVE</strong><span>Unofficial fan-made tracker. Poster imagery is retrieved from Wikipedia/Wikimedia. Automated catalog checks use TMDB. Trailer buttons open YouTube; no movies or episodes are hosted or streamed here.</span><button onClick={exportProgress}>Export your progress</button></footer>
  </main>;
}
